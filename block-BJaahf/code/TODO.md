...
```js
function intersection(arrays,arr1,ar2) {
let value=arrays.filter(elm=>{
   return  arr1.includes(elm)
})
 return value.filter(elm=>{
    return ar2.includes(elm)
 }) 
}
...
```js
function union(arrays,arr1,arr2) {
 let value=arrays.concat(arr1,arr2);
return value.filter((elm,index,arr)=>{
